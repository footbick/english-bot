...
async def send_next_step(user_id):
    ...
    try:
        ...
        if q_type == 'vocab':
            ...
            if cat and cat not in ["all", "mix"]:
                query = query.filter(Vocab.category == cat)
            ...
            if not target:
                await bot.send_message(user_id, "⚠️ No words found.")
                return
...
"def":"...",
"syn":"...",
"o":["{target.word}","similar1","similar2","similar3"],
"e_en":"Detailed explanation",
"e_ru":"Подробное объяснение"
}}
"""

            res = await ai_request(prompt, "Teacher. JSON ONLY.", json_mode=True)
            if not res:
                await bot.send_message(user_id, "⚠️ AI error. Try again.")
                return

            try:
                data = json.loads(res)
            except:
                await bot.send_message(user_id, "⚠️ Bad AI response.")
                return
            ...
            sess.update({
                'correct_id': opts.index(target.word),
                'exp': f"✅ <b>{target.word}</b>\n\n{data['e_en']}\n\n🇷🇺 {data['e_ru']}"
            })
            ...
            await bot.send_poll(
                ...
                opts,
                type='quiz',
                correct_option_id=sess['correct_id'],
                is_anonymous=False
            )
...
"c":0,
"e_en":"Detailed explanation",
"e_ru":"Подробный разбор"
}}
"""

            res = await ai_request(prompt, "Grammar Teacher. English only.", json_mode=True)
            if not res:
        ...
        await bot.send_message(user_id, "⚠️ Error. Try again.")
    finally:
        db.close()
...
async def v_menu(m: types.Message):
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="➕ Add New", callback_data="voc_add"),
...
async def v_start(cb: types.CallbackQuery):
    cat = cb.data.replace("voc_", "")  # ✅ FIX
    if cat == "add":
        await cb.message.answer("Send word or list (e.g. Apple, Banana)")
        return
...
async def list_words(cb: types.CallbackQuery):
    off = int(cb.data.split('_')[1])
    db = SessionLocal()
    words = db.query(Vocab).filter(Vocab.user_id == cb.from_user.id)\
        .order_by(desc(Vocab.id)).limit(8).offset(off).all()
    db.close()

    if not words and off == 0:
        await cb.answer("Empty.")
        return

    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"❌ {w.word}", callback_data=f"del_{w.id}_{off}")]
        for w in words
    ])

    if len(words) == 8:
        kb.inline_keyboard.append([
            InlineKeyboardButton(text="Next ➡️", callback_data=f"list_{off+8}")
        ])
...
async def spk_start(m: types.Message):
    ...
    if not res:
        await m.answer("AI error")
        return

    try:
        topic = json.loads(res).get('t', 'General topic')
    except:
        topic = "General topic"
...
async def handle_pdf(m: types.Message):
    st = await m.answer("⏳ Processing PDF...")
    ...
    res = await ai_request(
        f"Extract 5 items. JSON: {{\"items\":[{{\"w\":\"word\",\"d\":\"def\",\"c\":\"word/phrase/idiom\"}}]}}. Text: {text[:2000]}",
        "JSON ONLY.",
        json_mode=True
    )

    if not res:
        await st.edit_text("AI error")
        return

    try:
        items = json.loads(res).get('items', [])
    except:
        await st.edit_text("Bad AI response")
        return
    ...
    await st.edit_text(f"✅ Added {len(items)} items from PDF.")
...
async def manual_add(m: types.Message):
    if m.text.startswith("/") or m.text in [
        "📁 Upload PDF", "🎤 Speaking Practice",
        "📚 Vocabulary", "⚙️ Grammar Test", "📊 My Progress"
    ]:
        return
    ...
    for w in words:
        res = await ai_request(
            f"Define '{w}'. JSON: {{\"d\":\"def\",\"c\":\"word/phrase/idiom\"}}",
            "JSON ONLY.",
            json_mode=True
        )
    ...
    await m.answer(f"✅ Added {len(words)} items.")
