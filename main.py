    ...
    async with processing_lock:
        ...
        try:
            ...
            if q_type == 'vocab':
                ...
                prompt = f"Word: '{target.word}'. Define for B2. JSON: {{\"def\":\"...\", \"syn\":\"...\", \"o\":[\"{target.word}\",\"w1\",\"w2\",\"w3\"], \"e_en\":\"Why correct\", \"e_ru\":\"Почему верно\"}}"
                ...
                prompt = f"Topic: {topic}. B2. JSON: {{\"q\":\"... ____ ...\", \"o\":[\"a\",\"b\",\"c\",\"d\"], \"c\":0, \"e_en\":\"rule\", \"e_ru\":\"правило\"}}"
        ...
        except: await bot.send_message(user_id, "⚠️ Error. Re-select topic.")
        finally: db.close()
...
async def cmd_start(m: types.Message):
    ...
    await m.answer("🎯 Coach v5.1 Ready!", reply_markup=kb)
...
async def show_progress(m: types.Message):
    ...
    try:
        ...
        await m.answer(f"📊 <b>Ваш прогресс:</b>\n\nВсего слов в базе: {count}", parse_mode="HTML")
    finally: db.close()
...
async def v_start(cb: types.CallbackQuery):
    ...
    if cat == "add": await cb.message.answer("Send word or list (Apple, Banana)"); return
...
async def g_menu(m: types.Message):
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🔗 Complex Object", callback_data="gt_complex"), InlineKeyboardButton(text="🎲 Mixed", callback_data="gt_general")]
    ])
...
async def handle_pdf(m: types.Message):
    ...
    res = await ai_request(f"Extract 5 B2 items. JSON: {{\"items\":[{{\"w\":\"word\",\"d\":\"def\",\"c\":\"word/phrase/idiom\"}}]}}. Text: {text[:1500]}", "JSON ONLY.", json_mode=True)
    items = json.loads(res).get('items', []); db = SessionLocal()
    for i in items: db.add(Vocab(user_id=m.from_user.id, word=i['w'], definition=i['d'], category=i.get('c', 'word'), source="PDF"))
