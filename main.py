...
# --- 3. FUNCTIONS (Определяем всё ДО запуска main) ---
...
async def send_reminder():
    ...
    try:
        ...
        for u in users:
            try:
                await bot.send_message(u.user_id, "🔔 <b>Time for English!</b>\nПора сделать пару тестов и повторить слова.")
            except: pass
    finally:
        db.close()

async def ai_request(prompt, system_msg, json_mode=False, fast=True):
    """fast=True использует 8b модель (мгновенно), False - 70b (умно)"""
...
async def process_word_queue():
    """Фоновый воркер для обработки слов"""
    while True:
        user_id, word_id, word_text = await word_queue.get()
        db = SessionLocal()
        try:
            prompt = f"Define '{word_text}'. JSON: {{\"d\":\"def\", \"c\":\"word/phrase/phrasal_verb/idiom\"}}"
    ...
    try:
        ...
        if q_type == 'vocab':
            ...
            if not target:
                ...
                else: await bot.send_message(user_id, "⚠️ Category empty or processing."); return
        ...
        res = await ai_request(f"Topic: {topic}. JSON: {{\"q\":\".. ____ ..\",\"o\":[\"a\",\"b\",\"c\",\"d\"],\"c\":0,\"e_en\":\"eng\",\"e_ru\":\"рус\"}}", "Grammar.", True)
        data = json.loads(res)
...
async def cmd_start(m: types.Message):
    ...
    await m.answer("🎯 Coach v7.2 Active!", reply_markup=kb)
...
async def spk_menu(m: types.Message):
    ...
    res = await ai_request("5 B2 topics. JSON: {\"topics\":[\"T1\",\"T2\",\"T3\",\"T4\",\"T5\"]}", "JSON ONLY.", True, False)
...
async def spk_init(cb: types.CallbackQuery):
    topic = cb.data[7:]; q = await ai_request(f"Start conversation about {topic}. Max 2 sentences.", "Teacher.", False, False)
