...
class Vocab(Base):
    ...
    user_id = Column(BigInteger, index=True) # Добавлен индекс для скорости
    word = Column(String)
    definition = Column(Text)
    category = Column(String) 
    source = Column(String)

# Функция для проверки и создания колонок, если их нет
def init_db():
    Base.metadata.create_all(bind=engine)
    with engine.connect() as conn:
        # Проверяем наличие колонки user_id (критично для Postgres)
        try:
            conn.execute(text("SELECT user_id FROM vocab LIMIT 1"))
        except:
            logging.info("Adding user_id column to vocab table...")
            conn.execute(text("ALTER TABLE vocab ADD COLUMN user_id BIGINT"))
            conn.commit()
    ...
    def call():
        ...
        try:
            return client.chat.completions.create(
                ...
                model="llama-3.3-70b-versatile", response_format=fmt, timeout=15
    ...
    async with processing_lock:
        ...
        try:
            ...
            if q_type == 'vocab':
                ...
                if not target:
                    if not exclude:
                        await bot.send_message(user_id, f"⚠️ Category '{cat}' is empty. Send some words to the bot first!"); return
                    sess['used_items'] = []; target = query.order_by(func.random()).first()

                sess.setdefault('used_items', []).append(target.id)
                prompt = f"Word: '{target.word}'. Define for B2. JSON: {{\"def\":\"...\", \"syn\":\"...\", \"o\":[\"{target.word}\",\"w1\",\"w2\",\"w3\"], \"e_en\":\"Detailed explain\", \"e_ru\":\"Разбор\"}}"
                ...
                await bot.send_poll(user_id, "Select the word:", opts, type='quiz', correct_option_id=sess['correct_id'], is_anonymous=False)

            else:
                topic = sess.get('grammar_topic', 'general')
                prompt = f"Topic: {topic}. B2. JSON: {{\"q\":\"Sentence... ____\", \"o\":[\"a\",\"b\",\"c\",\"d\"], \"c\":0, \"e_en\":\"Detail\", \"e_ru\":\"Разбор\"}}"
            ...
            await bot.send_message(user_id, "⚠️ Error. Please try selecting the topic again.")
        finally: db.close()
...
async def cmd_start(m: types.Message):
    ...
    await m.answer("🎯 English Coach v4.0 Active!", reply_markup=kb)
...
async def list_words(cb: types.CallbackQuery):
    ...
    await cb.message.edit_text("Dictionary:", reply_markup=kb)
...
async def show_progress(m: types.Message):
    ...
    await m.answer(f"📈 Vocabulary: {count} items.")
