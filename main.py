...
class Vocab(Base):
    ...
    category = Column(String) # word, phrase, phrasal_verb, idiom
    ...
    def call():
        ...
        try:
            return client.chat.completions.create(
                ...
                model="llama-3.3-70b-versatile", response_format=fmt, timeout=35
        ...
        try:
            ...
            if q_type == 'vocab':
                ...
                if target:
                    ...
                    res = await ai_request(f"Word: {target.word}. JSON: {{\"d\":\"def\",\"s\":\"syn\",\"o\":[\"{target.word}\",\"w1\",\"w2\",\"w3\"],\"e_en\":\"eng explanation\",\"e_ru\":\"русский перевод\"}}", "Teacher.", True)
            ...
            instr = "Grammar Teacher. For Conditionals use types 0-3 and Mixed. For Passive use advanced forms."
            res = await ai_request(f"Topic: {topic}. UNIQUE task. JSON: {{\"q\":\".. ____ ..\",\"o\":[\"a\",\"b\",\"c\",\"d\"],\"c\":0,\"e_en\":\"eng\",\"e_ru\":\"рус\"}}", instr, True)
            data = json.loads(res)
            ...
            await bot.send_message(user_id, "⚠️ AI timeout. Please tap the button again.")
        finally: db.close()
...
async def cmd_start(m: types.Message):
    ...
    await m.answer("🎯 Coach v6.2 Active!", reply_markup=kb)
...
async def v_start(cb: types.CallbackQuery):
    ...
    if cat == "add": await cb.message.answer("Send any amount of words/phrases (one per line)."); return
...
async def spk_menu(m: types.Message):
    st = await m.answer("⏳ Generating topics..."); res = await ai_request("5 catchy B2 topics. JSON: {\"topics\":[\"T1\",\"T2\",\"T3\",\"T4\",\"T5\"]}", "JSON ONLY.", True)
...
async def spk_init(cb: types.CallbackQuery):
    topic = cb.data[7:]; q = await ai_request(f"Start conversation about {topic}. Be concise (max 2 sentences).", "Teacher.")
...
async def handle_voice(m: types.Message):
    ...
    resp = await ai_request(f"History: {history}. User: {trans}. Reply briefly (2 sentences max) and ask question.", "Teacher.")
...
async def handle_pdf(m: types.Message):
    ...
    # Авто-классификация из PDF
    res = await ai_request(f"Extract 8 items. Analyze: 1 word->'word', 2+->'phrase', phrasal verb->'phrasal_verb'. JSON: {{\"items\":[{{\"w\":\"word\",\"d\":\"def\",\"c\":\"cat\"}}]}}. Text: {text[:1500]}", "JSON ONLY.", True)
    ...
    db.commit(); db.close(); await st.edit_text(f"✅ Added {len(items)} items with auto-categorization.")
...
async def manual_add(m: types.Message):
    ...
    st = await m.answer(f"⏳ Processing {len(lines)} items..."); db = SessionLocal()
    added = 0
    for w in lines:
        # Авто-классификация при ручном вводе
        prompt = (f"Define '{w}'. Analyze it: "
                  f"1 word -> 'word', 2+ words -> 'phrase', phrasal verb -> 'phrasal_verb', idiom -> 'idiom'. "
                  f"JSON: {{\"d\":\"definition\", \"c\":\"category_name\"}}")
        res = await ai_request(prompt, "English Teacher Classifier.", True)
        if res:
            try:
                data = json.loads(res)
