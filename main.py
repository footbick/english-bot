...
async def send_next_step(user_id):
    ...
    try:
        ...
        # В режиме экзамена выбираем между вокабуляром и ЛЮБОЙ грамматикой
        q_type = random.choice(['vocab', 'grammar']) if is_ex else sess['type']

        if q_type == 'vocab':
            query = db.query(Vocab).filter(Vocab.user_id == user_id)
            if not is_ex and sess.get('vocab_category') != 'all':
                query = query.filter(Vocab.category == sess['vocab_category'])
            
            target = query.filter(~Vocab.id.in_(sess.get('used', []))).order_by(func.random()).first()
            
            if not target:
                if is_ex: q_type = 'grammar' # Если слов нет, добиваем экзамен грамматикой
                else: await bot.send_message(user_id, "⚠️ Category empty."); return
            
            if target:
                sess.setdefault('used', []).append(target.id)
                data = await ai_request(f"Word: {target.word}. JSON: {{\"d\":\"def\",\"s\":\"syn\",\"o\":[\"{target.word}\",\"w1\",\"w2\",\"w3\"],\"ru\":\"перевод\"}}", "Expert Teacher.", True)
                opts = data['o']
        ...
        # GRAMMAR BLOCK
        # Если это экзамен, просим AI выбрать ЛЮБУЮ тему грамматики B2-C1
        topic = "ALL GRAMMAR (Mixed)" if is_ex else sess.get('grammar_topic', 'general')
        data = await ai_request(f"Topic: {topic}. JSON: {{\"q\":\".. ____ ..\",\"o\":[\"a\",\"b\",\"c\",\"d\"],\"c\":0,\"e\":\"rule\"}}", "Grammar PhD.", True)
        ...
        await bot.send_message(user_id, "⚠️ AI timeout. Click again.", parse_mode=ParseMode.HTML)
    finally: db.close()
...
async def cmd_start(m: types.Message):
    ...
    await m.answer("🎯 Coach v8.2 Active!", reply_markup=kb)
...
async def v_menu(m: types.Message):
    ...
    await m.answer(f"Vocabulary (Total: {count}):", reply_markup=kb, parse_mode=ParseMode.HTML)

@dp.callback_query(F.data.startswith("voc_"))
async def v_start(cb: types.CallbackQuery):
    cat = cb.data.split("_")[1]
    if cat == "add": await cb.message.answer("Send words (one per line).", parse_mode=ParseMode.HTML); return
...
async def manual_add(m: types.Message):
    ...
    st = await m.answer(f"⏳ Processing {len(lines)} items...", parse_mode=ParseMode.HTML)
    ...
    for w in lines:
        ...
        data = await ai_request(f"Define '{w}'. JSON: {{\"d\":\"def\", \"c\":\"word/phrase/phrasal_verb/idiom\"}}", "Vocab Teacher.", True)
        if data:
            db.add(Vocab(user_id=m.from_user.id, word=w, definition=data.get('d'), category=data.get('c')))
            added += 1
            if added % 5 == 0: await st.edit_text(f"⏳ Progress: {added}/{len(lines)}...")
    db.commit(); db.close()
    await st.edit_text(f"✅ Success! Added/Updated {added} items.", parse_mode=ParseMode.HTML)
...
async def g_menu(m: types.Message):
    ...
    await m.answer("Choose Topic:", reply_markup=kb, parse_mode=ParseMode.HTML)
...
async def spk_menu(m: types.Message):
    st = await m.answer("⏳ Generating topics...", parse_mode=ParseMode.HTML)
    ...
    await st.delete(); await m.answer("Pick a topic:", reply_markup=kb, parse_mode=ParseMode.HTML)

@dp.callback_query(F.data.startswith("spk_st_"))
async def spk_init(cb: types.CallbackQuery):
    topic = cb.data[7:]; q = await ai_request(f"Conversation about {topic}. Max 2 sentences.", "Teacher.")
...
async def handle_voice(m: types.Message):
    ...
    st = await m.answer("👂 Listening...", parse_mode=ParseMode.HTML)
    ...
    await m.answer(f"🗣 {resp}", parse_mode=ParseMode.HTML)
...
async def exam_mode(m: types.Message):
    ...
    await m.answer("🏆 <b>Starting Exam (10 Questions - Vocab & All Grammar)</b>", parse_mode=ParseMode.HTML); await send_next_step(m.from_user.id)
