...
async def send_next_step(user_id):
    ...
    try:
        ...
        # GRAMMAR
        topic = "B2-C1 Mixed English Grammar" if is_ex else sess.get('grammar_topic', 'general')
        data = await ai_request(f"Topic: {topic}. JSON: {{\"q\":\"sentence ____ rest\",\"o\":[\"correct\",\"w1\",\"w2\",\"w3\"],\"c\":0,\"e\":\"detailed rule\"}}", "PhD Teacher.", True)
        ...
        await bot.send_message(user_id, "⚠️ AI timeout. Please try again.")
    finally: db.close()
...
async def cmd_start(m: types.Message):
    ...
    await m.answer("🎯 Coach v8.3 Active!", reply_markup=kb)
...
async def manual_add(m: types.Message):
    ...
    for w in lines:
        # Перезапись: удаляем старое слово
    ...
    db.commit(); db.close()
    await st.edit_text(f"✅ Success! Added/Updated {added} items.")
...
async def main():
    ...
    scheduler.add_job(send_reminder_job, CronTrigger(hour='9,12,15,18', minute=0))
    scheduler.start()
